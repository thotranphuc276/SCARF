...
def smplx_lbsmap_top_k(lbs_weights, verts_transform, points, template_points, source_points=None, K=1,
                       addition_info=None):
    ...
    xyz_neighbs_weight_conf = torch.exp(
        -torch.sum(torch.abs(xyz_neighbs_lbs_weight - xyz_neighbs_lbs_weight[..., 0:1, :]),
                   dim=-1) / weight_std2)  # (bs, n_rays*K, k_neigh)
...
class SCARF(nn.Module):
    def __init__(self, cfg, init_beta=None, device='cuda:0'):
        ...
        if self.cfg.use_mesh:  # hybrid: mesh as inside
            self.cond_dim = 1
            self.mlp_geo = GeoSIREN(input_dim=3, z_dim=self.cond_dim, hidden_dim=128, output_dim=3, device=self.device,
                                    last_op=torch.tanh, scale=self.cfg.mesh_offset_scale)
            self.mlp_tex = GeoSIREN(input_dim=3, z_dim=self.cond_dim, hidden_dim=128, output_dim=3, device=self.device,
                                    last_op=torch.nn.GELU(), scale=1)
            init_aabb = [-1., -1., -1., 1., 1., 1.]
            self.mlp_geo = NGPNet(aabb=init_aabb, input_dim=3, cond_dim=0, output_dim=3, last_op=torch.tanh,
                                  scale=self.cfg.mesh_offset_scale)
            self.mlp_tex = NGPNet(aabb=init_aabb, input_dim=3, cond_dim=0, output_dim=3, last_op=torch.nn.GELU(),
                                  scale=1)
        ...
        if self.cfg.use_nerf:
            ...
            if self.cfg.use_fine:
                if cfg.share_fine:
                    ...
                    self.nerf_fine = NeRF(freqs_xyz=cfg.freqs_xyz, freqs_dir=cfg.freqs_dir, use_view=cfg.use_view,
                                          deformation_dim=0)
    ...
    def set_smplx(self):
        ...
        if self.cfg.use_highres_smplx:
            ...
            subdiv_verts = self.smplx_verts[self.smplx_faces[nearest_faces, 0]] * b_coords[:, 0:1] + self.smplx_verts[
                self.smplx_faces[nearest_faces, 1]] * b_coords[:, 1:2] + self.smplx_verts[
                               self.smplx_faces[nearest_faces, 2]] * b_coords[:, 2:]
            ...
            for key in self.part_idx_dict.keys():
                ...
                highres_color = smplx_color[self.smplx_faces[nearest_faces, 0]] * b_coords[:, 0] + smplx_color[
                    self.smplx_faces[nearest_faces, 1]] * b_coords[:, 1] + smplx_color[
                                    self.smplx_faces[nearest_faces, 2]] * b_coords[:, 2]
        ...
        with torch.no_grad():
            ...
            if train:
                if self.cfg.train.precrop_iters > batch['global_step'] and not self.cfg.sample_patch_rays:
                    ...
                    coords = torch.stack(torch.meshgrid(torch.linspace(0, self.image_size - 1, self.image_size),
    ...
    def backward_skinning(self, batch, xyz, viewdir):
        ...
        posed_verts, _, _, joints_transform, curr_vertices_transform, shape_offsets, pose_offsets = \
            self.smplx(full_pose=batch['full_pose'], shape_params=self.beta.expand(batch_size, -1),
                       transl=batch.get('transl', torch.zeros([batch_size, 3], device=xyz.device)), return_T=True)
        ...
        vertices_transform_3x3 = torch.cat([vertices_transform[..., :3, :3], vertices_transform_3x1[..., None]],
                                           dim=-1)  # 3x4
        ...
        if self.cfg.use_highres_smplx:
            ...
            lbs_weights = (
                        util.batch_index_select(self.smplx.lbs_weights[None, ...], verts_index) * self.b_coords[None, :,
                                                                                                  :, None]).sum(
                2).squeeze()
        else:
    ...
    def query_model(self, batch, rays, xyz, viewdir, z_vals, noise_std, use_fine=False, render_cloth=False,
                    with_shape=False, get_normal=False):
        ## backward skinning
        ...
        if get_normal:
            ...
            normals, _, _ = fancy_integration(torch.cat([normal, sigma], dim=-1), z_vals, device=self.device,
                                              white_back=self.cfg.dataset.white_bg,
        ...
        if self.cfg.use_mesh:
            ...
            if render_cloth:
                if with_shape:
                    ...
                    rgb[:, :, -1, :3] = 1.
        rgbs, depths, weights = fancy_integration(torch.cat([rgb, sigma], dim=-1), z_vals, device=self.device,
                                                  white_back=self.cfg.dataset.white_bg,
                                                  last_back=self.cfg.use_mesh, clamp_mode='relu', noise_std=noise_std)
    ...
    def forward_nerf(self, batch, rays, train=False, render_cloth=False, with_shape=False):
        ...
        if self.cfg.n_importance > 0 and self.cfg.share_fine:
            with torch.no_grad():
                output = self.query_model(batch, rays, xyz, viewdir, z_vals, noise_std=noise_std,
                                          render_cloth=render_cloth, with_shape=with_shape)
        else:
            output = self.query_model(batch, rays, xyz, viewdir, z_vals, noise_std=noise_std, render_cloth=render_cloth,
        ...
        if self.cfg.use_fine and self.cfg.n_importance > 0:
            ...
            xyz_fine, viewdir_fine, z_vals_fine = self.sample_points(rays,
                                                                     z_coarse=z_coarse.reshape(-1, self.cfg.n_samples), \
                                                                     weights=weights.reshape(-1, self.cfg.n_samples)[
                                                                             ..., 1:-1].detach())  # (bs, n_rays, Kf)
            fine_output = self.query_model(batch, rays, xyz_fine, viewdir_fine, z_vals_fine, noise_std=noise_std,
                                           use_fine=True, render_cloth=render_cloth, with_shape=with_shape)
    ...
    def forward_mesh(self, batch, returnVerts=False, renderShape=False, renderDepth=False, background=None,
                     clean_offset=False):
        batch_size = batch['cam'].shape[0]
        output = {}
        ...
        if self.cfg.use_highres_smplx:
            ...
            posed_verts, _, _, = \
                self.smplx(full_pose=batch['full_pose'], shape_params=self.beta.expand(batch_size, -1),
                           transl=batch.get('transl', torch.zeros([batch_size, 3], device=self.device)), offset=offset)
        ...
        if renderShape:
            ...
            if self.cfg.use_outer_mesh:
                shape_image_outer = render_shape(vertices=trans_verts_outer.detach(),
        ...
        if self.cfg.use_nerf:
            ...
            if train:
                ...
                for i in range(self.image_size ** 2 // chunk):
                    ...
                    nerf_out = self.forward_nerf(batch, rays[:, chunk_idx], train=False, render_cloth=render_cloth,
                                                 with_shape=render_shape)
