...
class PoseModel(nn.Module):
    def __init__(self, dataset, optimize_cam=False, use_perspective=False,
        ...
        if self.use_deformation:
            ...
            for key in deformation_dict.keys():
                self.register_parameter(key.replace('cam', 'deformation'),
                                        torch.nn.Parameter(torch.zeros([1, deformation_dim])))
    ...
    def forward(self, batch):
        ...
        if self.use_deformation:
            batch['deformation_code'] = torch.cat(
                [getattr(self, f'{names[i]}_deformation_{frame_ids[i]}') for i in range(batch_size)])
        return batch
...
class Trainer(torch.nn.Module):
    def __init__(self, config=None):
        ...
        # Setup logger to log to both console and file
        log_file = os.path.join(self.cfg.output_dir, self.cfg.train.log_dir, 'train.log')
        logger.add(log_file, level="INFO")  # Log to file at INFO level
        logger.add(sys.stdout, level="INFO")  # Also log to stdout (console)

        # load model
        self.prepare_data()
        init_beta = self.train_dataset[0]['beta']
        self.model = SCARF(self.cfg, device=self.device, init_beta=init_beta).to(self.device)
        self.posemodel = PoseModel(dataset=self.train_dataset, optimize_cam=self.cfg.opt_cam,
                                   use_perspective=self.cfg.use_perspective,
                                   use_deformation=self.cfg.use_deformation & (self.cfg.deformation_type == 'opt_code'),
                                   deformation_dim=self.cfg.deformation_dim,
                                   ).to(self.device)
    ...
    def configure_optimizers(self):
        ...
        self.decay_steps = [1000, 5000, 10000, 50000];
        self.decay_gamma = 0.5
        self.scheduler = torch.optim.lr_scheduler.MultiStepLR(self.optimizer, milestones=self.decay_steps,
    ...
    def training_step(self, batch, batch_nb, test=False):
        ...
        if self.cfg.use_nerf:
            ...
            if self.cfg.loss.nerf_reg_normal_w > 0.:
                ...
                losses['nerf_reg_normal'] = self.cfg.loss.nerf_reg_normal_w * F.mse_loss(points_normal,
                                                                                         points_neighbs_normal)
                if self.cfg.use_fine and self.cfg.n_importance > 0 and not self.cfg.share_fine:
                    ...
                    losses['nerf_reg_normal'] += self.cfg.loss.nerf_reg_normal_w * F.mse_loss(points_normal_fine,
                                                                                              points_neighbs_normal_fine)
            losses['rgb'] = lossfunc.huber(opdict['rgbs'] * batch['mask_sampled'],
                                           batch['image_sampled'] * batch['mask_sampled']) * self.cfg.loss.w_rgb
            losses['alpha'] = lossfunc.huber(opdict['alphas'], gt_mask) * self.cfg.loss.w_alpha
            if self.cfg.use_fine and self.cfg.n_importance > 0 and not self.cfg.share_fine:
                losses['rgb_fine'] = lossfunc.huber(opdict['rgbs_fine'] * batch['mask_sampled'],
                                                    batch['image_sampled'] * batch[
                                                        'mask_sampled']) * self.cfg.loss.w_rgb
                ...
                if self.cfg.sample_patch_rays:
                    ...
                    rgb_patch = opdict['rgbs_fine'][:, :patch_size ** 2].reshape(-1, patch_size, patch_size, 3).permute(
                        0, 3, 1, 2)
                    rgb_patch_gt = batch['image_sampled'][:, :patch_size ** 2].reshape(-1, patch_size, patch_size,
                                                                                       3).permute(0, 3, 1, 2)
                    mask_patch = gt_mask[:, :patch_size ** 2].reshape(-1, patch_size, patch_size, 1).permute(0, 3, 1, 2)
                    if self.cfg.loss.w_patch_mrf > 0.:
                        losses['nerf_patch_mrf'] = self.mrf_loss(rgb_patch * mask_patch,
                                                                 rgb_patch_gt * mask_patch) * self.cfg.loss.w_patch_mrf
                    if self.cfg.loss.w_patch_perceptual > 0.:
                        losses['nerf_patch_perceptual'] = self.perceptual_loss(rgb_patch * mask_patch,
                                                                               rgb_patch_gt * mask_patch) * self.cfg.loss.w_patch_perceptual
        ...
        if self.cfg.use_mesh and self.cfg.opt_mesh:
            if self.cfg.loss.geo_reg:
                ...
                losses["reg_offset_fh"] = (opdict['offset'][:, exclude_idx] ** 2).sum(
                    -1).mean() * self.cfg.loss.reg_offset_w_face
                losses["reg_offset_hand"] = (opdict['offset'][:, self.model.part_idx_dict['hand']] ** 2).sum(
                    -1).mean() * self.cfg.loss.reg_offset_w_face * 9.
                if self.cfg.loss.use_new_edge_loss:
                    offset = opdict['mesh_offset']
                    new_verts = self.model.verts[None, ...].expand(offset.shape[0], -1, -1) + offset
                    losses["reg_edge"] = lossfunc.relative_edge_loss(new_verts, self.model.verts[None, ...].expand(
                        new_verts.shape[0], -1, -1),
                                                                     vertices_per_edge=self.model.verts_per_edge) * self.cfg.loss.reg_edge_w * 100.
                else:
                    losses["reg_edge"] = mesh_edge_loss(mesh) * self.cfg.loss.reg_edge_w

            if self.cfg.use_nerf:
                losses['mesh_skin_mask'] = lossfunc.huber(batch['skin_mask'] * opdict['mesh_mask'],
                                                          batch['skin_mask']) * self.cfg.loss.mesh_w_alpha_skin
                losses['mesh_inside_mask'] = (torch.relu(
                    opdict['mesh_mask'] - batch['mask'])).abs().mean() * self.cfg.loss.mesh_inside_mask
                losses['mesh_image'] = lossfunc.huber(batch['skin_mask'] * opdict['mesh_image'],
                                                      batch['skin_mask'] * batch['image']) * self.cfg.loss.mesh_w_rgb
                ...
                if self.cfg.loss.skin_consistency_type == 'verts_all_mean':
                    losses['mesh_skin_consistency'] = (tex - tex.detach().mean(1)[:, None,
                                                             :]).abs().mean() * self.cfg.loss.mesh_skin_consistency
                if self.cfg.loss.skin_consistency_type == 'verts_hand_mean':
                    ...
                    losses['mesh_skin_consistency'] = (tex[:, non_hand_idx] - tex[:, hand_idx].detach().mean(1)[:, None,
                                                                              :]).abs().mean() * self.cfg.loss.mesh_skin_consistency
                if self.cfg.loss.skin_consistency_type == 'render_nonskin_mean':
                    losses['mesh_skin_consistency'] = (batch['cloth_mask'] * (
                                opdict['mesh_image'] - tex.detach().mean(1)[:, :, None,
                                                       None]).abs()).mean() * self.cfg.loss.mesh_skin_consistency
                if self.cfg.loss.skin_consistency_type == 'render_hand_mean':
                    hand_idx = self.model.part_idx_dict['hand']
                    losses['mesh_skin_consistency'] = (batch['cloth_mask'] * (
                                opdict['mesh_image'] - tex[:, hand_idx].detach().mean(1)[:, :, None,
                                                       None]).abs()).mean() * self.cfg.loss.mesh_skin_consistency
                if self.cfg.loss.mesh_w_mrf > 0.:
                    losses['mesh_image_mrf'] = self.mrf_loss(opdict['mesh_image'] * batch['skin_mask'],
                                                             batch['image'] * batch[
                                                                 'skin_mask']) * self.cfg.loss.mesh_w_mrf
                if self.cfg.loss.mesh_w_perceptual > 0.:
                    losses['mesh_image_perceptual'] = self.perceptual_loss(opdict['mesh_image'] * batch['skin_mask'],
                                                                           batch['image'] * batch[
                                                                               'skin_mask']) * self.cfg.loss.mesh_w_perceptual
            else:
                losses['mesh_image'] = lossfunc.huber(opdict['mesh_image'], batch['image']) * self.cfg.loss.mesh_w_rgb
                losses['mesh_mask'] = lossfunc.huber(opdict['mesh_mask'], batch['mask']) * self.cfg.loss.mesh_w_alpha
                if self.cfg.loss.mesh_w_mrf > 0.:
                    losses['mesh_image_mrf'] = self.mrf_loss(opdict['mesh_image'],
                                                             batch['image']) * self.cfg.loss.mesh_w_mrf
                if self.cfg.loss.mesh_w_perceptual > 0.:
                    losses['mesh_image_perceptual'] = self.perceptual_loss(opdict['mesh_image'], batch[
                        'image']) * self.cfg.loss.mesh_w_perceptual
    ...
    def validation_step(self, data='train', batch=None, returnVis=False):
        ...
        visdict['shape_image'] = self.model.forward_mesh(batch, renderShape=True, background=batch['image'])[
            'shape_image']
        if self.cfg.use_outer_mesh:
            visdict['shape_outer_image'] = self.model.forward_mesh(batch, renderShape=True, background=batch['image'])[
                'shape_outer_image']
